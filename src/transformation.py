...
def transform_etf_data(df_etf_raw):
    ...
    df_etf.index = pd.to_datetime(df_etf.index).tz_localize(None).normalize().as_unit("us")
...
def transform_wibor_data(df_wibor_raw):
    ...
    df_wibor.index = pd.to_datetime(df_wibor.index).tz_localize(None).normalize().as_unit("us")
